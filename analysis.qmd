...
# TODO - load exported CSVs instead to reduce embed load
...
noDataOutMessage = html`<div style="width:475px;padding-top:25px;display:flex;"><p style="margin:auto;">No data for student flows <strong>out of ${basisSelect}</strong> in <strong>${yearSelect}</strong>.</p><div>`
```

```{ojs}
//| label: bycountryorigins
...
~~What we need is to match each row with its inverse: to find rows X and Y such that `origin_country_name[X] == dest_country_name[Y]` and `origin_country_name[Y] == dest_country_name[X]`. If we can pivot wider on that combination, we can calculate net flows.~~

~~To do this, we need a key that matches the two countries. I'm going to sort the two alphabetically to create that key.~~

This is a network analysis problem, so let's use the `igraph` package to turn our data into data frames of vertices (country info) and edges (student flows).

```{r}

# first, let's get country centroid data from {rnaturalearth}
sf::sf_use_s2(FALSE)
...
# now, we'll get a list of our countries to be the vertices, and add the 
bind_rows(
  national_data_joined %>%
...
# join 'em up
country_centroids %>%
  right_join(country_list, by = "country_code") ->
country_vertices

# list of graphs (one for each year)
national_data_joined %>%
  select(origin_country_code, dest_country_code, year, value, magnitude,
    qualifier) %>%
  # some duplicates have snuck in with the "unknown countries" categories
  # and can be safely discarded
  distinct(origin_country_code, dest_country_code, year, .keep_all = TRUE) %>%
  filter(!is.na(origin_country_code), !is.na(dest_country_code), !is.na(year)) %>%
  split(.$year) %>%
  # graph_from_data_frame(directed = TRUE, vertices = country_vertices) ->
  map(graph_from_data_frame, directed = TRUE, vertices = country_vertices) %>%
  tibble(year = as.numeric(names(.)), graph = .) ->
country_edges



# national_data_joined %>%
#   select(-ends_with("continent")) %>%
#   # now create a two-country key and direction for the pivot
#   mutate(
#     direction =
#       if_else(origin_country_name < dest_country_name, "toA", "toB"),
#     pairkey = paste(
#       pmin(origin_country_code, dest_country_code),
#       pmax(origin_country_code, dest_country_code),
#       sep = "_")) %>%
#   pivot_wider(
#     id_cols = c(pairkey, year), names_from = direction,
#     values_from = c(starts_with("origin"), starts_with("dest"), value, magnitude, qualifier),
#     values_fn = function(x) x[[1]]) %>% View()
```
