...
national_data %>%
  ...
  select(year = YEAR, continent, country_code = COUNTRY_ID, country_name, value,
  ratio, magnitude = MAGNITUDE, qualifier = QUALIFIER) ->
netflows
```

Let's also bring Human Development Index data in to compare against. That's the `IDX.HDI` (or, using a "revised method", `IDX.HDI.REV`)

```{r}
# from https://hdr.undp.org/en/indicators/137506
# but not iso3c codes!
read_csv(here("data", "hdi.csv"), skip = 5) %>%
  filter(!is.na(`HDI Rank`)) %>%
  select(country = Country, starts_with("19"), starts_with("20")) %>%
  pivot_longer(-country, names_to = "year", values_to = "hdi") %>%
  mutate(year = as.integer(year)) ->
hdi

# need a better join key! iso3c?
netflows %>%
  left_join(hdi, by = c("country_name" = "country", "year" = "year")) %>%
  select(year, continent, country_code, country_name, value, hdi)
