...
# write out to disk
st_write(flows_all, here("data", "flows_all.gpkg"))
st_write(flows_all %>% filter(year == 2010), here("data", "flows_2010.gpkg"))
```

Let's take one year - 2010 - and start visualising flows.

```{r}
flows_2010 <- flows_all %>% filter(year == 2010L)

st_crs(flows_2010) <- 4326

flows_all %>%
  filter(origin_country_code == "AUS", year == 2010L) %>%
#   pull(path) %>%
#   st_crs()
  {
    ggplot(.) +
      geom_sf(aes(geometry = path, colour = value)) +
      coord_sf(crs = st_crs(.))
  }
```