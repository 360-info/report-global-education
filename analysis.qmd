...
setdiff(country_list$country_code, boundaries$country_code) %>%
  ...
  print(n = Inf)
# i'm going to eyeball this one! 12.76555 29.91269
```

I'll add it manually (we don't need the centroids to be too precise):
...
```{r}
# bring the country centroid info in and create lines between countries
national_data_joined %>%
  left_join(country_centroids, by = c("origin_country_code" = "country_code")) %>%
  rename(origin_pt = geometry) %>%
  left_join(country_centroids, by = c("dest_country_code" = "country_code")) %>%
  rename(dest_pt = geometry) %>%
  st_as_sf(crs = st_crs(4326)) %>%
  mutate(segment = st_cast(
    st_union(origin_pt, dest_pt, by_feature = TRUE),
    "LINESTRING") ->
flows_all
```
